...
def node_to_list(node):
    ...
    # If/Else Statement
        ...
        # The 'if' header spans from its own start to the end of the test expression
        header_end = node.test.end_lineno
        header_metadata = NodeMetadata('if', start, header_end)
        
        res = [header_metadata, node_to_list(node.test)]

        res.append([NodeMetadata('root', None, None)] + [node_to_list(n) for n in node.body])
        if node.orelse:
            res.append([NodeMetadata('root', None, None)] + [node_to_list(n) for n in node.orelse])
        return res
    ...
    match tree[0].name:
        ...
        case 'if':
            ...
            if len(tree) == 4:
                ...
                # Line number for else is unknown, so we have to make the string ourselves
                print_original_code(' '* (tab_depth * 4) + 'else:')
                
                # TODO print_debug_info('# else:')
                tab_depth += 1
...
def parse(code):
    ...
    full_list = [NodeMetadata('root', None, None)] + [node_to_list(node) for node in tree.body]
    return full_list
