...
@assert_stack_decorator(n=2)
def ABS(res, a, depth, tab_depth):
    # res = |a|
    print_debug_info(res, '= abs(', a, ')', tab_depth=tab_depth)
...
def node_to_list(node):
    ...
    # Binary Operations
        ...
        op_map = {ast.Add: '__add__', ast.Sub: '__sub__', ast.Mult: '__mul__', ast.Div: '__div__'}
        op_name = op_map.get(type(node.op), type(node.op).__name__)
        return [NodeMetadata('function', 0, 1, op_name), [node_to_list(node.left), node_to_list(node.right)]]
        ...
        case 'function':
            ...
            if node.source in safe_functions:
                ...
                if not hidden:
                    ...
                    ret = dfs(inline_tree, True, True, tab_depth)
                assert ret == res_var
