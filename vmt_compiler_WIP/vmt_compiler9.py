...
def get_all_locals(func_def, protected_names):
    """Finds all unique variable names that aren't arguments."""
    locals_found = set()
    for node in ast.walk(func_def):
        if isinstance(node, ast.Name) and node.id not in protected_names:
            locals_found.add(node.id)
    return locals_found

def handle_call_inline(node, call_args, res_var, depth):
...
def dfs(tree, p=True, store_only_if_p_is=True, tab_depth=0, true_false_mode=None, res_var=None):
    ...
    match tree[0].name:
        ...
        case 'function':
            ...
            print_extra_info('# Inlining function call to', func_def, tab_depth=tab_depth)

            inline_tree, number_of_extra_variables = handle_call_inline(node, call_args, res_var, depth)
            depth += number_of_extra_variables

            ret = dfs(inline_tree, p, store_only_if_p_is, tab_depth + 1, true_false_mode)
            assert ret == res_var
